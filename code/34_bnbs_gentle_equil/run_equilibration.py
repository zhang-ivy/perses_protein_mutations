...
with open(os.path.join(args.outdir, f"{args.phase}_solvated.cif"), "r") as f:
    pdb = app.PDBxFile(f)
...
if args.gentle:
    ...
    stages = [
        {'EOM': 'minimize', 'n_steps': 0, 'temperature': 300*unit.kelvin, 'ensemble': 'NPT', 'restraint_selection': None, 'force_constant': 0*unit.kilocalories_per_mole/unit.angstrom**2, 'collision_rate': 1/unit.picoseconds, 'timestep': 2*unit.femtoseconds},
        {'EOM': 'MD', 'n_steps': 5000000, 'temperature': 300*unit.kelvin, 'ensemble': 'NPT', 'restraint_selection': None, 'force_constant': 0*unit.kilocalories_per_mole/unit.angstrom**2, 'collision_rate': 1/unit.picoseconds, 'timestep': 2*unit.femtoseconds},
    ]
