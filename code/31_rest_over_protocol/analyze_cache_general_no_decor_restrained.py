...
parser.add_argument('--length', type=int, help='length of simulation in ns')
...
def get_dihedrals(i, name, length, out_dir, htf, dihedral_indices_new, dihedral_indices_old):
    ...
    nc = open_netcdf(os.path.join(out_dir, f"{i}_{phase}_{name.lower()}_{length}ns_restrained.nc"))
    nc_checkpoint = open_netcdf(os.path.join(out_dir, f"{i}_{phase}_{name.lower()}_{length}ns_restrained_checkpoint.nc"))
...
cys_dihedral = ['N', 'CA', 'CB', 'SG']

dihedral_atoms = []
for aa_name in [args.old_aa_name, args.new_aa_name]:
    if aa_name in ["PHE", "TYR", "TRP", "GLU", "LYS", "ARG", "GLN", "ASN", "LEU", "MET", "HIS", "HIE", "HID", "HIP"]:
        ...
        dihedral_atoms.append(cys_dihedral)
...
# Discard first 1.5 ns and then save every 5 snapshots
subset_pos = all_pos_hybrid[1501::5] # Make array of hybrid positions for 100 uncorrelated indices
_logger.info(f"subset_pos shape: {subset_pos.shape}")
with open(os.path.join(out_dir, f"{i}_{phase}_{name.lower()}_{length}ns_snapshots_restrained.npy"), 'wb') as f:
    np.save(f, subset_pos)

#  Save box vectors corresponding to each snapshot
subset_box_vectors = box_vectors[1501::5]
_logger.info(f"subset_box_vectors shape: {subset_box_vectors.shape}")
with open(os.path.join(out_dir, f"{i}_{phase}_{name.lower()}_{length}ns_box_vectors_restrained.npy"), "wb") as f:
    np.save(f, subset_box_vectors)

# Plot 
dihedrals_new = dihedrals[0]
dihedrals_old = dihedrals[1]
plot_dihedrals(dihedrals_old, os.path.join(out_dir, f"{i}_{phase}_{name.lower()}_{length}ns_{args.old_aa_name.lower()}_correlated_restrained.png"))
plot_time_series(dihedrals_old, n_iter, os.path.join(out_dir, f"{i}_{phase}_{name.lower()}_{length}ns_{args.old_aa_name.lower()}_timeseries_restrained.png"))
plot_dihedrals(dihedrals_new, os.path.join(out_dir, f"{i}_{phase}_{name.lower()}_{length}ns_{args.new_aa_name.lower()}_correlated_restrained.png"))
plot_time_series(dihedrals_new, n_iter, os.path.join(out_dir, f"{i}_{phase}_{name.lower()}_{length}ns_{args.new_aa_name.lower()}_timeseries_restrained.png"))
