...
def run_gentle_equilibration(topology, positions, system, stages, platform_name='CUDA'):
    ...
    for i, parameters in enumerate(stages):
        ...
        dcd_reporter = app.DCDReporter(
            file=os.path.join(args.outdir, f"traj_{i+1}.dcd"),
    ...
    openmm.app.PDBxFile.writeFile(topology, positions, open(os.path.join(args.outdir, "equilibrated.cif"), "w"), keepIds=True)

    # Save the box vectors
    with open(os.path.join(args.outdir, 'equilibrated_box_vectors.npy'), 'wb') as f:
        np.save(f, box_vectors)

# Load topology and positions
print("Loading topology and positions", file=sys.stderr)
with open("/data/chodera/zhangi/perses_benchmark/neq/15/249/solvated.cif", "r") as f:
    pdb = app.PDBxFile(f)
...
with open("/data/chodera/zhangi/perses_benchmark/neq/15/249/solvated.xml", "r") as f:
...
if args.gentle:
    ...
    stages = [
        ...
        {'EOM': 'MD', 'n_steps': 20000000, 'temperature': 300*unit.kelvin, 'ensemble': 'NPT', 'restraint_selection': None, 'force_constant': 0*unit.kilocalories_per_mole/unit.angstrom**2, 'collision_rate': 2/unit.picoseconds, 'timestep': 2*unit.femtoseconds},
    ]
else:
    print("Running normal equil", file=sys.stderr)
    stages = [
        {'EOM': 'minimize', 'n_steps': 0, 'temperature': 300*unit.kelvin, 'ensemble': 'NPT', 'restraint_selection': None, 'force_constant': 0*unit.kilocalories_per_mole/unit.angstrom**2, 'collision_rate': 1/unit.picoseconds, 'timestep': 4*unit.femtoseconds},
        {'EOM': 'MD', 'n_steps': 10000000, 'temperature': 300*unit.kelvin, 'ensemble': 'NPT', 'restraint_selection': None, 'force_constant': 0*unit.kilocalories_per_mole/unit.angstrom**2, 'collision_rate': 1/unit.picoseconds, 'timestep': 4*unit.femtoseconds},
    ]
