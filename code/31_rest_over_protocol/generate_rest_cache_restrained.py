...
if args.sim_number == 1:
    ...
    name_other = args.new_aa_name
    state = 0
elif args.sim_number == 2:
    phase = 'apo'
    name = args.new_aa_name
    name_other = args.old_aa_name
    state = 1
...
# Load positions from other endstate's cache
with open(os.path.join(args.dir, f"{i}_{phase}_{name_other.lower()}_{length}ns_snapshots.npy"), "rb") as f:
    snapshot_cache = np.load(f)
    hybrid_positions = snapshot_cache[-1]

with open(os.path.join(args.dir, f"{i}_{phase}_{name_other.lower()}_{length}ns_box_vectors.npy"), "rb") as f:
    box_vector_cache = np.load(f)
    box_vectors = unit.Quantity(value=box_vector_cache[-1][0], unit=unit.nanometer)

# Add restraint to unique new atoms for lambda = 0 endstate (and unique old atoms for lambda = 1 endstate)
topology = htf.hybrid_topology
atom_indices = list(htf._atom_classes['unique_new_atoms']) if state == 0 else list(htf._atom_classes['unique_old_atoms'])
#atom_indices_str = [str(index) for index in atom_indices]
#selection = 'index ' + ' or index '.join(atom_indices_str)
#_logger.info(f"Restraint selection: {selection}")

#traj = md.Trajectory(hybrid_positions, topology)
...
REST_system.addForce(custom_cv_force)
...
sampler_state =  SamplerState(hybrid_positions, box_vectors=box_vectors)
...
# Run t-repex
reporter_file = os.path.join(args.dir, f"{i}_{phase}_{name.lower()}_{length}ns_restrained.nc")
