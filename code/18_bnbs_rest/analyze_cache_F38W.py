...
def get_dihedrals(i, name, length, out_dir, htf, dihedral_indices_new, dihedral_indices_old):
    ...
    with open(os.path.join(out_dir, f"{i}_{phase}_{name.lower()}_{length}ns_snapshots_all_new.npy"), 'wb') as f:
        np.save(f, all_pos_new)
    
    with open(os.path.join(out_dir, f"{i}_{phase}_{name.lower()}_{length}ns_snapshots_all_old.npy"), 'wb') as f:
        np.save(f, all_pos_old)
...
with open(os.path.join(out_dir, f"{i}_{phase}_{name.lower()}_{length}ns_snapshots_all.npy"), 'wb') as f:
