...
indices_old = [atom.index for atom in list(htf._topology_proposal.old_topology.residues())[args.resid].atoms() if atom.name in dihedral_atoms[0]]
indices_new = [atom.index for atom in list(htf._topology_proposal.new_topology.residues())[args.resid].atoms() if atom.name in dihedral_atoms[1]]
