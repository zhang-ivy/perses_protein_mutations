...
parser.add_argument('restraint', type=str, help="either 'heavy' or 'CA', 'heavy-not-res', 'heavy-not-unique', None ")
parser.add_argument('force_constant', type=float, help='in kcal/molA^2')
parser.add_argument('resid', type=int, help='resid of residue being mutated')
...
force_constant = args.force_constant*unit.kilocalories_per_mole/unit.angstrom**2
_logger.info(force_constant)
...
if args.restraint != 'None':
    ...
    elif args.restraint == 'heavy-not-res':
        heavy_atom_indices = [atom.index for atom in topology.atoms if atom.residue.name not in solvent_types and atom.element.name != 'hydrogen']
        res_indices = [atom.index for atom in topology.atoms if atom.element.name != 'hydrogen' and atom.residue.resSeq == args.resid and atom.residue.chain.index == 0]
        atom_indices = list(set(heavy_atom_indices) - set(res_indices))
    else:
