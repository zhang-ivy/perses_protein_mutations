...
with open(os.path.join(out_dir, f"{i}_{args.phase}_{args.endstate}.pickle"), 'rb') as f:
    htf = pickle.load(f)

if args.name == 'THR': # r-htf
    indices_old = [6, 8, 10, 11]
    indices_new = [6, 8, 10, 18]
elif args.name == 'SER': # r-htf
    indices_old = [11, 10, 7, 6]
    indices_new = [15, 10, 7, 6]
elif args.name == 'ALA': # r-htf
    #indices_old = [6, 7, 8, 10] # phi angle
    #indices_new = [12, 10, 7, 6]
    indices_old = [6, 8, 10, 11]
    #indices_new = [0, 6, 7, 10] # phi angle
    indices_new = [6, 8, 10, 18]
dihedrals, n_iter, all_pos_hybrid = get_dihedrals_all_replicas(i, aa, length, out_dir, htf, indices_new, indices_old)
...
for j, replica in tqdm(enumerate(dihedrals)):
    ...
    plot_dihedrals(dihedrals_old, os.path.join(out_dir, f"{i}_{args.phase}_{aa}_{length}_{j}_{old_aa}_correlated.png"))
    uncorrelated_old = plot_time_series(dihedrals_old, n_iter, os.path.join(out_dir, f"{i}_{args.phase}_{aa}_{length}_{j}_{old_aa}_timeseries.png"))
    ...
    if j == 0:
        if args.name.lower() == new_aa:
            uncorrelated_indices = uncorrelated_new
        elif args.name.lower() == old_aa:
        ...
        with open(os.path.join(out_dir, f"{i}_{args.phase}_{aa}_{args.length}ns_snapshots.npy"), 'wb') as f:
