...
class RESTTopologyFactoryV2(HybridTopologyFactory):
    ...
    def scaling_expression(self, nb=False):
        if not nb:
            last_line = '_is_rest = delta(identifier); _not_rest = delta(1-identifier); _is_inter = delta(2-identifier);'
        else:
            last_line = '_is_rest = delta(identifier1 + identifier2); _not_rest = delta(2 - (identifier1 + identifier2)); _is_inter = delta(1 - (identifier1 + identifier2));'

        out = f"scale_factor = rest_scale*_is_rest + not_rest_scale*_not_rest + inter_scale*_is_inter; \
               not_rest_scale = 1.; \
               {last_line} \
               "
        return out
    ...
    def _add_custom_nonbonded_force_terms(self):
        ...
        formatted_expression = ["(U_electrostatics) + (U_sterics);", 

                             "U_electrostatics = {ONE_4PI_EPS0} * chargeProd / r;", 
        ...
        custom_nonbonded_force.setUsesPeriodicBoundaryConditions(True)

        # Set whether to use dispersion correction
        if self._og_system_forces['NonbondedForce'].getUseDispersionCorrection() and self._use_dispersion_correction:
            custom_nonbonded_force.setUseDispersionCorrection(True)
        else:
            custom_nonbonded_force.setUseDispersionCorrection(False)
    ...
    def _add_custom_bond_force_terms(self):
        ...
        formatted_expression = ["(U_electrostatics) + (U_sterics);", 
                             ...
                             "U_electrostatics = {ONE_4PI_EPS0} * chargeProd * elec_scale / r;", 
        ...
        # Set the appropriate switching function parameters
        switch_bool = self._og_system_forces['NonbondedForce'].getUseSwitchingFunction()
        custom_bond_force.setUseSwitchingFunction(switch_bool)
        if switch_bool:
            switching_distance = self._og_system_forces['NonbondedForce'].getSwitchingDistance()
            custom_bond_force.setSwitchingDistance(switching_distance)

        # Set the appropriate cutoff distance
        r_cutoff = self._og_system_forces['NonbondedForce'].getCutoffDistance()
        if self._nonbonded_method != openmm.NonbondedForce.NoCutoff:
            custom_bond_force.setCutoffDistance(r_cutoff)

        # Set the appropriate nonbonded method and whether to use PBCs
        custom_bond_force.setNonbondedMethod(openmm.NonbondedForce.CutoffPeriodic)
        custom_bond_force.setUsesPeriodicBoundaryConditions(True)

        # Set whether to use dispersion correction
        if self._og_system_forces['NonbondedForce'].getUseDispersionCorrection() and self._use_dispersion_correction:
            custom_bond_force.setUseDispersionCorrection(True)
        else:
            custom_bond_force.setUseDispersionCorrection(False)
    ...
    def is_solvent(self, particle_index, positive_ion_name="NA", negative_ion_name="CL", water_name="HOH"):
    ...
    def _add_custom_bonds(self):
        ...
        for exception_idx in range(og_nb_force.getNumExceptions()):
            ...
            self._out_system_forces['CustomBondForce'].addBond(p1, p2, *params)
